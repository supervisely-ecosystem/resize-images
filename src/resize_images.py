...
def resize_images(api: sly.Api, task_id, context, state, app_logger):
    ...
    app.logger.info(
        "Result Project is created (name={!r}; id={})".format(dst_project.name, dst_project.id)
    )
    ...
    for dataset in api.dataset.get_list(src_project.id):
        ...
        for batch in sly.batched(ds_images, batch_size=10):
            ...
            resized_images_info = api.image.upload_nps(
...
#     sly.logger.info(
#         "Script arguments",
#         extra={"TEAM_ID": TEAM_ID, "WORKSPACE_ID": WORKSPACE_ID, "PROJECT_ID": PROJECT_ID},
#     )
